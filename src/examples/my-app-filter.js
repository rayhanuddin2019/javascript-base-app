var tCore = dFanatics.getInstance({
   ...
   * Register Core module
   **/
  tCore.register("blog:filter:searchbyname", function (sandbox, moduleSettings) {
    sandbox.init = function () {
      window.addEventListener("DOMContentLoaded", function () {
          // search by name
          var title_component = {
            name     : "search_by_name",
            container: document.querySelector(".post-title-search"),
            dom      : {
              search_by_name : document.querySelector(".ml-search-by-name"),                          
            },
            request_options: {
            ...
            view(model) {
              const titleHTML = model.title_content.reduce(
                (html, post) => html + `<li class="blue-color"><a data-loc-name="" class="location_title blue-color" href="#">${post.title}</a></li>`,
                ""
              );
              return `${titleHTML}`;
            },          
            async controller(component) {                
              ...
              await fetch(url, {
                ...
                component.model.title_content = data;
                console.log(data); 
              });                  
  ...
  sandbox.router = function () {       
      // search_by_name     
      if(sandbox.componentsByName['search_by_name']){     
        sandbox.componentsByName['search_by_name'].dom.search_by_name.addEventListener('keyup', function(e){
          sandbox.componentsByName['search_by_name'].model.filter_options.text    = e.target.value;
          sandbox.showComponent('search_by_name'); 
        });
      }       
    };
